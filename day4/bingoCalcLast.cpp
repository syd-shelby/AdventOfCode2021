...
int main(int argc, char *argv[]) {
	...
	//play game
	Board lastWin = boards[0];
	int lastNum;
	for (int i =0; i < nums.size(); i++){
		markBoards(nums[i], boards);
		vector<int> winners;
		for (int j =0; j<boards.size(); j++){ //lol should be acting like a list and not array to make this faster
		...
		for (int k = winners.size() -1; k>=0; k--){
			boards.erase(boards.begin()+winners[k]);
		}	
	
	}

	cout<<lastWin.score()*lastNum<<endl;
